...
const defaultForm: TaskFormData = {
  name: '', icon: '🏠', frequency: 'daily', frequency_value: 1, max_per_cycle: 1, points: 5, assigned_to: 'both', color_tag: '#0D9488',
};
        ...
        <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) { setForm(defaultForm); setEditingId(null); } }}>
          ...
          <DialogContent className="rounded-2xl max-w-sm mx-auto">
            ...
            <div className="space-y-4">
              <Input placeholder="Task name" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} className="rounded-xl" />
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {EMOJI_OPTIONS.map(e => (
                    <button key={e} onClick={() => setForm({ ...form, icon: e })} className={`text-2xl p-1.5 rounded-lg ${form.icon === e ? 'bg-primary/20 ring-2 ring-primary' : 'hover:bg-muted'}`}>
                      {e}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {COLOR_OPTIONS.map(c => (
                    <button key={c} onClick={() => setForm({ ...form, color_tag: c })} className={`w-8 h-8 rounded-full ${form.color_tag === c ? 'ring-2 ring-primary ring-offset-2 ring-offset-background' : ''}`} style={{ backgroundColor: c }} />
                  ))}
                </div>
              </div>
        ...
        {tasks.map((task, i) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground">{task.points}pts · {task.frequency} · max {task.max_per_cycle}/day</p>
            </div>
            <div className="flex gap-1">
              <button onClick={() => handleEdit(task)} className="p-2 rounded-xl hover:bg-muted text-muted-foreground"><Pencil className="w-4 h-4" /></button>
              <button onClick={() => handleDelete(task.id)} className="p-2 rounded-xl hover:bg-destructive/10 text-destructive"><Trash2 className="w-4 h-4" /></button>
            </div>
          </motion.div>
        ))}
      </div>
